...
for file in os.listdir(path):
    ...
    if file.endswith(".tif") or file.endswith(".tiff"):
        ...
        image=tiff.imread(path+'/'+file)

        gray_img=graycode(image)#Returns grayscale image
        name=remove_ext(file)
        changed_image=color_threshold(gray_img)#Returns image after threshokding to remove unclassified pixels in annotations
        sett,distinct_colours=unique_colours(changed_image)#Returns a set of all colours found in analysis and also the colors available in the particular image
